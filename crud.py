...
async def passengerclass():
 ...
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/passengerclass/


#***Get con Filtro Path
@app.get("/passengerclass/{id}")
async def passengerclass(id:int):
    passenger=filter (lambda passenger: passenger.id == id, Passenger_list)  #Función de orden superior
    try:
        return list(passenger)[0]
    except:
        return{"error":"No se ha encontrado el pasajero"}
    
     # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/passengerclass/1
...
async def passengerclass(id:int):
 ...
 # En el explorador colocamos la raiz de la ip: http://127.0.0.1:8000/passengerclass/?id=1
...
async def passengerclass(passenger:Passenger):
    ...
    for index, saved_passenger in enumerate(Passenger_list):
        if saved_passenger.id == passenger.id:  #Si el Id del usuario guardado es igual al Id del usuario nuevo
            return {"error":"el pasajero ya existe ya existe"}
    else:
        Passenger_list.append(passenger)
        return passenger
    
    #http://127.0.0.1:8000/passengerclass/
   
   
    #***Put
@app.put("/passengerclass/")
async def passengerclass(passenger:Passenger):
    ...
    for index, saved_passenger in enumerate(Passenger_list):
        if saved_passenger.id == passenger.id:  #Si el Id del usuario guardado es igual al Id del usuario nuevo
           Passenger_list[index] = passenger  #accedemos al indice de la lista que hemos encontrado y actualizamos con el nuevo usuario
           found=True
    ...
    #http://127.0.0.1:8000/passengerclass/
        ...
        #***Delete
@app.delete("/passengerclass/{id}")
async def passengerclass(id:int):
    
    found=False     #Usamos bandera found para verificar si hemos encontrado el usuario 
    
    for index, saved_passenger in enumerate(Passenger_list):
        if saved_passenger.id ==id:  #Si el Id del usuario guardado es igual al Id del usuario nuevo
           del Passenger_list[index]  #Eliminamos al indice de la lista que hemos encontrado 
           found=True
           return "El registro se ha eliminado"
       
    if not found:
        return {"error":"No se ha eliminado el pasajero"}
    
    #http://127.0.0.1:8000/passengerclass/1 