...
async def get_all_users():
    users = db_client.actividad9.pasajeros.find()
    return [user_schema(user) for user in users]

#***Get Usuario por ID
@router.get("/userdb2/{user_Name}")
async def get_user(user_Name: str):
    user = db_client.actividad9.pasajeros.find_one({"Name": user_Name})
    if user:
...
async def get_user_by_query(Name: str):
    user = db_client.actividad9.pasajeros.find_one({"Name": Name})
    if user:
        return user_schema(user)
    else:
        raise HTTPException(status_code=404, detail="User not found")
...
async def userdb2(user:User):
    ...
    id= db_client.actividad9.pasajeros.insert_one(user_dict).inserted_id
    ...
    new_user=  user_schema(db_client.actividad9.pasajeros.find_one({"_id":id}))
...
async def update_user(user_Name: str, updated_user: User):
    ...
    existing_user = db_client.actividad9.pasajeros.find_one({"Name": user_Name})
    ...
    if existing_user:
        ...
        result = db_client.actividad9.pasajeros.update_one({"Name": user_Name}, {"$set": updated_user.dict()})
...
async def delete_user(Name: str):
    result = db_client.actividad9.pasajeros.delete_one({"Name": Name})
