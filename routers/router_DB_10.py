...
async def usersclass():
    users = list(db_client.actividad10.alumnos.find())
...
async def get_user(user_username: str):
    user = db_client.actividad10.alumnos.find_one({"username": user_username})
    if user:
...
async def get_user_by_query(username: str):
    user = db_client.actividad10.alumnos.find_one({"username": username})
    if user:
        return user_schema(user)
    else:
        raise HTTPException(status_code=404, detail="User not found")
...
async def userdb(user:User):
    ...
    id= db_client.actividad10.alumnos.insert_one(user_dict).inserted_id
    ...
    new_user=  user_schema(db_client.actividad10.alumnos.find_one({"_id":id}))
...
async def update_user(user_username: str, updated_user: User):
    ...
    existing_user = db_client.actividad10.alumnos.find_one({"username": user_username})
    ...
    if existing_user:
        ...
        result = db_client.actividad10.alumnos.update_one({"username": user_username}, {"$set": updated_user.dict()})
...
async def delete_user(username: str):
    result = db_client.actividad10.alumnos.delete_one({"username": username})
